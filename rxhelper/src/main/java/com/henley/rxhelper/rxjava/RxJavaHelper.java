...
public class RxJavaHelper {
     ...
     * @return
     */
    public static <T> Disposable concat(BaseObserver<T> baseObserver, ObservableSource<? extends ObservableSource<? extends T>> sources) {
        Observable<T> observable = Observable.concat(sources)
                .unsubscribeOn(Schedulers.io()) // 取消订阅
                .subscribeOn(Schedulers.io()) // 指定上游发送事件的线程(只有第一次指定有效)
                .observeOn(AndroidSchedulers.mainThread());// 指定下游接收事件的线程(每次指定都有效)
        return subscribe(observable, baseObserver);
     ...
     * @return
     */
    public static <T> Disposable concatArray(BaseObserver<T> baseObserver, ObservableSource<? extends T>... sources) {
